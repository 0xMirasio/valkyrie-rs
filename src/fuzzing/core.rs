...
#[derive(Debug)]
pub(crate) struct CoverageMapRef {
    pub(crate) ptr: *mut u8,
    pub(crate) len: usize,
    pub(crate) prev_loc: usize,
}

#[derive(Debug)]
pub(crate) struct SharedCoverageHandle {
    pub(crate) state: *mut CoverageMapRef,
}
...
pub(crate) fn run_valkyrie_prepared(vk: &mut Valkyrie) -> Result<ExitKind> {
...
pub(crate) fn install_shared_block_coverage(
    vk: &mut Valkyrie,
...
pub(crate) fn clear_coverage_map(state: &mut CoverageMapRef) {
    if state.len == 0 {
        return;
    }
